...
type Props = NativeStackScreenProps<RootStackParamList, 'DetalhesDetrito'>;

const DetalhesDetritoScreen = ({ navigation, route }: Props) => {
  const { id } = route.params;
  const { detritos, deleteDetrito } = useDetritosContext();
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.historyCard}>
          ...
          {detrito.registros && detrito.registros.length > 0 ? (
            ...
            <Text style={styles.description}>Nenhum registro historico disponivel para este detrito.</Text>
          )}
        </View>
